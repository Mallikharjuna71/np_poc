...
df = df.distinct().dropDuplicates(subset=[primary_keys[table_name]]).dropna(how='all', subset=[primary_keys[table_name]]).withColumn(f'{table_name}_hash_key', sha2(concat_ws(df['article_id'], df['author_id']), 256))
